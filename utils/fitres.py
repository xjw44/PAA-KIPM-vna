...
def finefit(f, z, fr_0, take_fr = False):
    """
    ...
    if take_fr:
        lb = [     0,      0, -np.inf, -np.inf,-1*np.pi, -np.inf, -np.inf]
        ub = [np.inf, np.inf,  np.inf,  np.inf,   np.pi,  np.inf,  np.inf]
        p0 = [abs(Qr_1), abs(Qc_hat_mag_1), a_1.real, a_1.imag, phi_1, tau_1, Imtau_1]
        resfunc = lambda f_proj, Qr,  Qc_hat_mag, a_real, a_imag, phi, tau, Imtau: resfunc8(f_proj, fr_0, Qr,  Qc_hat_mag, a_real, a_imag, phi, tau, Imtau)
    else:
        lb = [min(f), 0, 0, -np.inf, -np.inf,-1*np.pi, -np.inf, -np.inf]
        ub = [max(f), np.inf, np.inf, np.inf, np.inf, np.pi, np.inf, np.inf]
        p0 = [fr_1, Qr_1, Qc_hat_mag_1, a_1.real, a_1.imag, phi_1, tau_1, Imtau_1]
        resfunc = resfunc8
    # print(p0)
    bounds = (lb,ub)
    fparams, fcov = opt.curve_fit(resfunc, xdata, ydata, p0=p0, bounds=bounds, maxfev=10000)
    ...
    if take_fr:
        fr_fine = fr_0
        Qr_fine = fparams[0]
        Qc_hat_mag_fine = fparams[1]
        a_fine = fparams[2]+1j*fparams[3]
        phi_fine = fparams[4]
        tau_fine = fparams[5] + 1j*fparams[6]
    else:
        fr_fine = fparams[0]
        Qr_fine = fparams[1]
...
def sweep_fit(f, z, nsig=3, fwindow=5e-4, pdf_rewrite=True, additions=[], filename='test', start_f=None, stop_f=None):
    """
...
##                if cp > delta and f[i] > (min(f)+2*fwindow):
    ...
    print('peaklist_len', len(peaklist))
    ...
    for i in range(len(peaklist)):
        ...
        try:
            print(f[peaklist[i]])
            fr_list[i], Qr_list[i], Qc_hat_mag_list[i], a_list[i], phi_list[i], tau_list[i], Qc_list[i], _ = finefit(f_curr, z_curr, f[peaklist[i]])
        ...
        if pdf_rewrite == True:
            ...
            fitwords = r"$f_{r}$ = " + str(fr_list[i]) + "\n" + r"$Q_{r}$ = " + str(Qr_list[i]) + "\n" + r"$Q_{c}$ = " + str(Qc_list[i]) + "\n" + r"$Q_{i}$ = " + str(Qi_list[i]) + "\n" + r"$\phi_{0}$ = " + str(phi_list[i]) + "\n" + r"$a$ = " + str(a_list[i]) + "\n" + r"$\tau$ = " + str(tau_list[i]) + "\n"
